...
const MainLayout = ({
  ...
  const translateUI = async (targetLang) => {
    try {
      ...
      setTranslatedText(baseText); // fallback
    }
  };
    ...
    <div className="main-layout">
      <nav className="navbar">
        <div className="nav-left">
          ...
          <span className="product-name">Smart Farm 360</span>
        </div>
 ...
 <button
            ...
            {showLangMenu && (
              <div className="dropdown-menu">
                {Object.entries(languageLabels).map(([code, name]) => (
                  <div key={code} className="dropdown-item" onClick={() => handleLanguageChange(code)}>
                    {name}
                  </div>
                ))}
              </div>
          ...
          {!isAuthenticated ? (
            <Link to="/login" className="nav-link login-btn">{t.login}</Link>
          ) : (
            <div className="profile-section">
              ...
              {showProfileMenu && (
                <div className="profile-menu">
                  ...
                  <div className="profile-menu-item">{t.help}</div>
                  <div className="profile-menu-item">{t.sendFeedback}</div>
                  <div className="profile-menu-item" onClick={onLogout}>{t.logout}</div>
                </div>
              )}
            </div>
          )}
      ...
      <footer className="footer">
        ...
        <div className="footer-bottom">
          <p>&copy; 2025 Smart Farm 360. {t.allRightsReserved}</p>
        </div>
      </footer>
    </div>
  );
